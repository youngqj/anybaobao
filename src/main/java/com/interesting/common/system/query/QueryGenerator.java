package com.interesting.common.system.query;
...
import com.interesting.common.system.util.JeecgDataAutorUtils;
import com.interesting.common.system.util.JwtUtil;
import com.interesting.common.system.vo.SysPermissionDataRuleModel;
...
public class QueryGenerator {
    ...
    private static final String BEGIN_ = "At";
    private static final String END_ = "Ed";
    /**
     * 数字类型字段，拼接此后缀 接受多值参数
     */
    ...
    public static void main(String[] args) throws ParseException {
		Date parse = getTime().parse("2021-08-06 12:22:1");
		int i = 0;
	}
...
//						 可以设置左右模糊或全模糊，因人而异
    ...
    private static void doIntervalQuery(QueryWrapper<?> queryWrapper, Map<String, String[]> parameterMap, String type, String filedName, String columnName) throws ParseException {
        ...
        if (parameterMap != null && parameterMap.containsKey(filedName + BEGIN_)) {
            beginValue = parameterMap.get(filedName + BEGIN_)[0].trim();
            addQueryByRule(queryWrapper, columnName, type, beginValue, QueryRuleEnum.GE);

        }
        if (parameterMap != null && parameterMap.containsKey(filedName + END_)) {
            endValue = parameterMap.get(filedName + END_)[0].trim();
            addQueryByRule(queryWrapper, columnName, type, endValue, QueryRuleEnum.LE);
        }
                    ...
                    for (int i = 0; i < conditions.size(); i++) {
                        ...
                        if (oConvertUtils.isNotEmpty(rule.getField())
                            ...
                            if ("date".equals(rule.getType())) {
                                queryValue = DateUtils.str2Date(rule.getVal(), DateUtils.date_sdf);
                            } else if ("datetime".equals(rule.getType())) {
                                queryValue = DateUtils.str2Date(rule.getVal(), DateUtils.datetimeFormat);
                            }
        ...
        if (rule.equals(QueryRuleEnum.IN) && !propertyType.equals(String.class)) {
            ...
            if (propertyType.equals(String.class)) {
                ...
                if (dateStr.length() == 10) {
                    addEasyQuery(queryWrapper, name, rule, DateUtils.str2Date(dateStr, DateUtils.date_sdf));
                } else {
                    addEasyQuery(queryWrapper, name, rule, DateUtils.str2Date(dateStr, DateUtils.datetimeFormat));
                }
            } else {
