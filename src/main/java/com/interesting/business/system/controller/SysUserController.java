...
import com.interesting.business.system.model.SysUserSysDepartModel;
import com.interesting.business.system.service.*;
import com.interesting.business.system.vo.SysDepartUsersVO;
import com.interesting.business.system.vo.SysUserRoleVO;
import com.interesting.common.api.vo.Result;
import com.interesting.common.aspect.annotation.PermissionData;
...
import com.interesting.modules.util.AESUtils;
...
public class SysUserController {
    ...
    @ApiOperation(value = "用户管理-获取用户列表数据", notes = "用户管理-获取用户列表数据")
    public Result<List<SysUser>> queryPageListByEnterpriseInfoId(
            @ApiParam(value = "企业id", name = "enterpriseInfoId", required = false) @RequestParam(required = false) String enterpriseInfoId,
            @ApiParam(value = "真实姓名", name = "name", required = false) @RequestParam(required = false) String name,
            @ApiParam(value = "用户id", name = "userId", required = false) @RequestParam(required = false) String userId
    ) {
        List<SysUser> pageList = sysUserService.list(
                new QueryWrapper<SysUser>().
                        eq(StringUtils.isNotEmpty(enterpriseInfoId), "enterprise_info_id", enterpriseInfoId)
                        .like(StringUtils.isNotEmpty(name), "realname", name)
                        .eq(StringUtils.isNotEmpty(userId), "id", userId)
        );
    ...
    public Result<SysUser> add(@RequestBody SysUser s) {
        Result<SysUser> result = new Result<SysUser>();
        try {
            s.setCreateTime(new Date());//设置创建时间
            String salt = oConvertUtils.randomGen(8);
            s.setSalt(salt);

            String password = s.getPassword();
            try {
                password = AESUtils.aesDecrypt(password, AESUtils.key);
            } catch (Exception e) {
                throw new InterestingBootException("密码解密失败！");
            }
            s.setPassword(password);

            String passwordEncode = PasswordUtil.encrypt(s.getUsername(), s.getPassword(), salt);
            s.setPassword(passwordEncode);
            s.setStatus(1);
            s.setIzDelete(CommonConstant.IZ_DELETE_0);
            sysUserService.addUserWithRole(s, s.getSelectedroles());
            sysUserService.addUserWithDepart(s, s.getSelecteddeparts());
            result.success("添加成功！");
        } catch (Exception e) {
    ...
    public Result<SysUser> edit(@RequestBody SysUser jsonObject) {
        Result<SysUser> result = new Result<SysUser>();
        try {
            SysUser sysUser = sysUserService.getById(jsonObject.getId());
            baseCommonService.addLog("编辑用户，id： " +jsonObject.getId(), CommonConstant.LOG_TYPE_2, 2);
            if (sysUser == null) {
                result.error500("未找到对应实体");
            } else {
                jsonObject.setUpdateTime(new Date());
                sysUserService.editUserWithRole(jsonObject, jsonObject.getSelectedroles());
//                sysUserService.editUserWithDepart(jsonObject, jsonObject.getSelecteddeparts());
...
//                json.put("avatar", item.getSysUser().getAvatar());
     ...
     */
    //@RequiresRoles({"admin"})
    @RequestMapping(value = "/deleteUserInDepart", method = RequestMethod.GET)
     ...
     */
    //@RequiresRoles({"admin"})
    @RequestMapping(value = "/deleteUserInDepartBatch", method = RequestMethod.GET)
    ...
    public Result<?> appUserList(@RequestParam(name = "keyword", required = false) String keyword,
        ...
        try {
            ...
            query.eq(SysUser::getIzDelete, CommonConstant.IZ_DELETE_0);
            if (oConvertUtils.isNotEmpty(username)) {
     ...
     */
    //@RequiresRoles({"admin"})
    @RequestMapping(value = "/deleteRecycleBin", method = RequestMethod.GET)
