...
import com.interesting.common.util.MD5Util;
import com.interesting.common.util.oConvertUtils;
import com.interesting.business.system.entity.SysDepartPermission;
import com.interesting.business.system.entity.SysPermission;
import com.interesting.business.system.entity.SysPermissionDataRule;
import com.interesting.business.system.entity.SysRolePermission;
import com.interesting.business.system.model.SysPermissionTree;
import com.interesting.business.system.model.TreeModel;
import com.interesting.business.system.service.*;
import com.interesting.business.system.util.PermissionDataUtil;
...
public class SysPermissionController {
    ...
    public Result<List<SysPermissionTree>> list() {
        ...
        try {
            ...
            query.eq(SysPermission::getIzDelete, CommonConstant.IZ_DELETE_0);
...
//	}
    ...
    @ApiOperation(value = "菜单管理-查询全部菜单", notes = "菜单管理-查询全部菜单")
    public Result<?> getUserPermissionByToken(
        ...
        try {
            //直接获取当前用户不适用前端token
            LoginUser loginUser = FilterContextHandler.getUserInfo();
            if (oConvertUtils.isEmpty(loginUser)) {
                return Result.error("请登录系统！");
            }
            List<SysPermission> metaList = null;
            if (id != null) {
                metaList = sysPermissionService.queryByUserIdAndParentId(loginUser.getUsername(), id);
            } else {
     ...
     * @return
     */
    @RequestMapping(value = "/getUserPermissionParentByToken", method = RequestMethod.GET)
    @ApiOperation(value = "菜单管理-查询根目录菜单", notes = "房间管理-查询根目录菜单")
    public Result<?> getUserPermissionParentByToken() {
        Result<JSONObject> result = new Result<JSONObject>();
        try {
            //直接获取当前用户不适用前端token
            LoginUser loginUser = FilterContextHandler.getUserInfo();
            ...
            List<SysPermission> metaList = sysPermissionService.queryParentByUser(loginUser.getUsername());
            //添加首页路由
            //update-begin-author:taoyan date:20200211 for: TASK #3368 【路由缓存】首页的缓存设置有问题，需要根据后台的路由配置来实现是否缓存
            if (!PermissionDataUtil.hasIndexPage(metaList)) {
                SysPermission indexMenu = sysPermissionService.list(new LambdaQueryWrapper<SysPermission>().eq(SysPermission::getName, "首页")).get(0);
            ...
            jsonObject.put("titleName", "园区智能运营平台");
            jsonObject.put("menuOne", menujsonArray);
            json.put("menu", jsonObject);
            //按钮权限（用户拥有的权限集合）
            json.put("auth", authjsonArray);
            //全部权限配置集合（按钮权限，访问权限）
            json.put("allAuth", allauthjsonArray);
     ...
     * @return
     */
    @RequestMapping(value = "/getSystemMenuByParentId", method = RequestMethod.GET)
    @ApiOperation(value = "菜单管理-查询子菜单，单个", notes = "菜单管理-查询子菜单，单个")
    public Result<List<SysPermissionTree>> getSystemMenuByParentId(@RequestParam("parentId") String parentId) {
        Result<List<SysPermissionTree>> result = new Result<>();
        try {
            LoginUser loginUser = FilterContextHandler.getUserInfo();
            if (oConvertUtils.isEmpty(loginUser)) {
                throw new InterestingBootException("请登录系统！");
            }
            List<SysPermission> list = sysPermissionService.queryByUserIdAndParentId(loginUser.getUsername(), parentId);
     ...
     * @return 返回 key-value 的 Map
     */
    @GetMapping("/getSystemMenuByParentIds")
    @ApiOperation(value = "菜单管理-查询子菜单，批量", notes = "菜单管理-查询子菜单，批量")
    public Result getSystemMenuByParentIds(@RequestParam("parentIds") String parentIds) {
        try {
            LambdaQueryWrapper<SysPermission> query = new LambdaQueryWrapper<>();
            List<String> parentIdList = Arrays.asList(parentIds.split(","));
            query.in(SysPermission::getParentId, parentIdList);
    ...
    private JSONObject getPermissionJsonObject(SysPermission permission) {
        ...
        if (permission.getMenuType().equals(CommonConstant.MENU_TYPE_2)) {
            ...
            meta.put("menuUrl", permission.getMenuUrl());
     ...
     */
    //@RequiresRoles({ "admin" })
    @RequestMapping(value = "/deletePermissionRule", method = RequestMethod.GET)
