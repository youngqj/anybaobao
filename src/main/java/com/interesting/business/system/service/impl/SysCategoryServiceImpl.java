...
import com.interesting.business.system.mapper.SysCategoryMapper;
...
public class SysCategoryServiceImpl extends ServiceImpl<SysCategoryMapper, SysCategory> implements ISysCategoryService {
	...
	@Override
	public List<TreeSelectModel> queryListByCode(String pcode) throws InterestingBootException {
		...
		if(oConvertUtils.isNotEmpty(pcode)) {
			...
			if(list==null || list.size() ==0) {
				throw new InterestingBootException("该编码【"+pcode+"】不存在，请核实!");
			}
			if(list.size()>1) {
				throw new InterestingBootException("该编码【"+pcode+"】存在多个，请核实!");
			}
	...
	@Override
	public List<TreeSelectModel> queryListByCodeAndModuleType(String orderId, String pid) throws InterestingBootException {
		...
		return baseMapper.queryListByPid(pid,map);
	}
	...
	public void deleteSysCategory(String ids) {
		...
		if (xqFilesService.lambdaQuery().in(XqFiles::getNodeId, allIds)
				.count() > 0) {
			throw new InterestingBootException("该分类下已有文件，无法删除");
		}
